...
test.describe
  .skip('Sweetlink v2 legacy headed integration tests', () => {
    // Kept as a manual exploratory script, but skipped by default because it uses
    // repo-level daemon state, sync CLI calls, and a detached headed daemon. The
    // maintained LLM/evidence coverage lives in _harness.ts-based specs.
    ...
    test.describe('Phase 1: Daemon Infrastructure', () => {
      ...
      test('responsive screenshots produce 3 breakpoints', async () => {
        ...
        const files = fs
          ...
          .filter((f: string) => f.startsWith('responsive-'));
    ...
    test.describe('Phase 3: Diffing & Annotation', () => {
      ...
      test('annotated screenshot has ref labels', async () => {
        ...
        console.log(
          `  Annotated screenshot saved: ${outPath} (${(stat.size / 1024).toFixed(0)}KB)`
        );
      });
    });
    ...
    test.describe('Phase 4: Ring Buffers', () => {
      ...
      test('console buffer live update — trigger burst and verify capture', async () => {
        ...
        console.log(
          `  Console entries: ${countBefore} → ${countAfter} (+${countAfter - countBefore})`
        );
      });
    });
    ...
    test.describe('Live Console Feedback Loop', () => {
      ...
      test('watch network buffer during page reload', async () => {
        ...
        const entries = after.entries as Array<{
          method: string;
          url: string;
          status: number;
          duration: number;
        ...
        recent.forEach((e) => {
          ...
          console.log(
            `  ${statusColor} ${e.status} ${e.method.padEnd(4)} ${urlDisplay} (${e.duration}ms)`
          );
        });
      });
    });
