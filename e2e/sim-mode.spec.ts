...
test.describe('sim ios', () => {
  test.skip(
    !hasBootedIos(),
    'No booted iOS Simulator — boot one with `xcrun simctl boot "iPhone 15"` to run this test.'
  );

  test('records the simulator screen into an h264 mp4', async () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'sl-sim-ios-'));
    try {
      const result = await cli(['sim', 'ios', '--label', 'home-screen', 'sleep 2'], cwd);
      ...
      try {
        ...
        /* ffprobe missing or failed — file size check is enough */
      }
    } finally {
...
test.describe('sim ios — error paths (no booted simulator)', () => {
  ...
  test.skip(
    hasBootedIos(),
    'A simulator is currently booted; this test verifies the no-sim error path.'
  );
...
test.describe('sim android', () => {
  ...
  test('records the emulator screen when one is online', async () => {
    test.skip(
      !hasOnlineAdbDevice(),
      'No online Android device — boot an emulator to run this test.'
    );
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'sl-sim-android-'));
    try {
      const result = await cli(['sim', 'android', '--label', 'home-screen', 'sleep 2'], cwd);
      expect(result.exitCode, result.stderr).toBe(0);
      const dir = path.join(cwd, '.sweetlink', 'sim');
